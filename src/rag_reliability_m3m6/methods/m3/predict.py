...
Запуск:
  python scripts/m3m6/run_m3.py --mode zero_shot --split val --limit 10
...
from rag_reliability_m3m6.common.llm_client import JudgeClient
from rag_reliability_m3m6.common.run_meta import save_run_yaml
from rag_reliability_m3m6.common.schemas import Pred, load_cases, save_preds
...
def get_system_prompt(cfg: dict, prompt_file: str | None = None) -> str:
    ...
    if mode == "few_shot":
        ...
        examples = yaml.safe_load(open("configs/m3m6/few_shot.yaml", encoding="utf-8"))["examples"]
...
def main() -> None:
    ...
    if args.concurrency > 1:
        ...
        from rag_reliability_m3m6.common.async_llm import AsyncJudgeClient
