...
def save_run_yaml(out_dir: str | Path, cfg: dict, **extra) -> None:
    payload = {"config": _redact(cfg), "git_hash": git_hash(),
               "profile": cfg.get("profile", "local"), **extra}
    out = Path(out_dir)
    out.mkdir(parents=True, exist_ok=True)
    (out / "run.yaml").write_text(yaml.safe_dump(payload, allow_unicode=True, sort_keys=False),
                                  encoding="utf-8")
