...
def _m3(name: str) -> BuildCommand:
    def build(ctx: CommandContext) -> list[str]:
        if name in ("m3_openai", "m3_openai_judge"):
            m3_mode = "zero_shot"
            backend = "openai" if name == "m3_openai" else "openai_judge"
        else:
...
METHODS: dict[str, MethodSpec] = {
    "dummy_direct": MethodSpec("dummy_direct", "Dummy — direct", "dummy", "direct", _dummy("direct"), "dummy"),
    "dummy_marker": MethodSpec("dummy_marker", "Dummy — marker", "dummy", "marker", _dummy("marker"), "dummy"),
    "prompt_direct": MethodSpec("prompt_direct", "Zero-shot prompt — direct", "prompt", "direct", _prompt("direct"), "prompt", ("MLX model",)),
    "prompt_marker": MethodSpec("prompt_marker", "Zero-shot prompt — marker", "prompt", "marker", _prompt("marker"), "prompt", ("MLX model",)),
    "lora_direct": MethodSpec("lora_direct", "LoRA — direct", "lora", "direct", _lora("direct"), "lora", ("results/adapters_direct",)),
    "lora_marker": MethodSpec("lora_marker", "LoRA — marker", "lora", "marker", _lora("marker"), "lora", ("results/adapters_marker",)),
    "lettucedetect": MethodSpec("lettucedetect", "LettuceDetect features", "lettucedetect", None, _lettucedetect, "lettucedetect", ("results/lettucedetect/classifier.joblib",)),
