...
## Текущие результаты (этапы −1 … −0.25, псевдо-корпус)

- Метод 3: few_shot f1_reliable **0.824** (val) — лучший вариант; GEPA-light его не превзошёл
  (medium не эскалировался по стоп-правилу); 100 % извлечение из logprobs.
- Метод 6: contradiction — рабочий faith-сигнал (Δ+0.31 на текущем корпусе); semantic
  entropy ненадёжна как faithfulness-признак (абляция thr×N, [`docs/10`](docs/10_gepa_stage.md) §4).
- Главный негативный результат: off_topic faith-путаница — ограничение 7B-судьи (не
  лечится ни промптом, ни few-shot); проверка на крупном backbone — на local-этапе.
- **Визуализации со всеми разборами: [`docs/viz/`](docs/viz/)** — дашборд, интерактивный
  отчёт, фигуры, скриншоты эксплорера + [объяснение результатов](docs/viz/README.md) и
  [справочник метрик](docs/viz/METRICS.md).

## Дальше

Local-этап (реальный корпус кураторов + vLLM): боевые H1/H5-прогоны — см.
чекбоксы в [`docs/05_tasks.md`](docs/05_tasks.md) и перенос в [`docs/11`](docs/11_viz_scale.md) §6.
