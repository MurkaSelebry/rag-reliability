...
def test_log_run_local_store(tmp_path):
    ...
    cfg = {"profile": "cloud", "llm": {"api_key": "sk-secret", "model": "m"},
           "m3": {"mode": "few_shot", "seed": 0}}
    rep = tmp_path / "report_val.json"
    rep.write_text('{"f1_macro_reliable": 0.82}')
    run_id = log_run(tracking_uri=uri, experiment="m3", run_name="few_shot/val",
                     cfg=cfg, metrics={"f1_macro_reliable": 0.82},
                     artifacts=[rep], tags={"split": "val", "variant": "few_shot"})
...
def test_log_run_twice_and_missing_artifact(tmp_path):
    ...
    uri = f"file:{tmp_path/'mlruns'}"
    cfg = {"profile": "local", "llm": {"api_key": "sk-x", "model": "m"},
           "m3": {"mode": "zero_shot", "seed": 1}}
    missing = tmp_path / "does_not_exist.json"  # отсутствующий путь — терпимо
    run_id1 = log_run(tracking_uri=uri, experiment="m3", run_name="a",
                      cfg=cfg, metrics={"f1_macro_reliable": 0.5},
                      artifacts=[missing], tags={"split": "val"})
    run_id2 = log_run(tracking_uri=uri, experiment="m3", run_name="b",
                      cfg=cfg, metrics={"f1_macro_reliable": 0.7},
                      artifacts=None, tags={"split": "val"})
