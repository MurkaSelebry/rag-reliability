...
поверх `src.common.results_index.build_index`; в отрисовке ничего «на глаз» не считается.
Тяжёлый plotly импортируется лениво внутри main.

Запуск:
  python -m tools.make_report --root . --out artifacts/report/index.html [--splits val test]
"""
...
from src.common.results_index import ResultsIndex, build_index
from tools.make_figs import KIND_ORDER, f1_threshold_curve, reliability_bins
...
def variant_summary(runs: pd.DataFrame) -> pd.DataFrame:
    ...
    for variant, grp in m3.groupby("variant"):
        ...
        row["gap_val_test"] = (rv - rt) if (rv is not None and rt is not None
                                            and pd.notna(rv) and pd.notna(rt)) else float("nan")
        out_rows.append(row)

    df = pd.DataFrame(out_rows)
    df = df.sort_values("f1_reliable_val", ascending=False,
...
def _sec_leaderboard(idx: ResultsIndex, first: bool) -> tuple[str, bool]:
    ...
    header = ["variant", "f1 reliable val", "f1 reliable test", "gap val−test",
              "f1 faith val", "f1 faith test", "f1 rel val", "f1 rel test", "seed"]
    ...
    hi = "#ffd9d0"
    lo = "#f7f7f7"
    row_colors = [hi if (isinstance(g, (int, float)) and pd.notna(g) and g > GAP_HIGHLIGHT)
                  else lo for g in gaps]
    fill = [row_colors for _ in header]

    fig = go.Figure(data=[go.Table(
        header=dict(values=[f"<b>{h}</b>" for h in header],
                    fill_color="#2b3a55", font=dict(color="white"), align="left"),
...
def _sec_grouped_bar(idx: ResultsIndex, first: bool) -> tuple[str, bool]:
    ...
    metrics = [("reliable", "f1_macro_reliable"), ("faith", "f1_macro_faith"),
               ("rel", "f1_macro_rel")]
    ...
    def series(full: str, split: str) -> list:
        ...
        for v in variants:
            ...
            vals.append(float(g[full].iloc[0]) if len(g) and pd.notna(g[full].iloc[0])
                        else None)
        return vals
    ...
    for m_i, (short, full) in enumerate(metrics):
        for split in ("val", "test"):
            fig.add_bar(x=variants, y=series(full, split), name=f"{split}",
                        visible=(m_i == 0),
    ...
    for m_i, (short, _full) in enumerate(metrics):
        ...
        vis[m_i * 2] = True
        vis[m_i * 2 + 1] = True
        buttons.append(dict(label=short, method="update",
                            args=[{"visible": vis},
                                  {"title": f"f1-macro ({short}): val vs test"}]))
    fig.update_layout(
        barmode="group", title="f1-macro (reliable): val vs test",
        updatemenus=[dict(buttons=buttons, direction="down", x=1.0, xanchor="right",
                          y=1.15, showactive=True)],
        yaxis=dict(title="f1-macro", range=[0, 1]), height=420,
...
def _sec_box(idx: ResultsIndex, first: bool) -> tuple[str, bool]:
    ...
    for f_i, field in enumerate(fields):
        for v_i, v in enumerate(variants):
            ...
            fig.add_trace(go.Box(
                x=[k for k in sub["kind"]], y=list(sub[field]),
                name=v, legendgroup=v, marker_color=palette[v_i % len(palette)],
                visible=(f_i == 0),
            ))
    n_v = len(variants)
    buttons = []
    for f_i, field in enumerate(fields):
        ...
        for j in range(n_v):
            vis[f_i * n_v + j] = True
        buttons.append(dict(label=field, method="update",
                            args=[{"visible": vis},
                                  {"title": f"{field} по kind (все варианты)"}]))
    fig.update_layout(
        boxmode="group", title="p_faith по kind (все варианты)",
        xaxis=dict(categoryorder="array", categoryarray=KIND_ORDER),
        yaxis=dict(title="вероятность", range=[-0.05, 1.05]),
        updatemenus=[dict(buttons=buttons, direction="down", x=1.0, xanchor="right",
                          y=1.18, showactive=True)],
        height=480, margin=dict(l=40, r=20, t=70, b=90),
    )
...
def _sec_heatmap_kind(idx: ResultsIndex, first: bool) -> tuple[str, bool]:
    ...
    def trace(piv: pd.DataFrame, visible: bool) -> go.Heatmap:
        ...
        return go.Heatmap(
            ...
            colorscale="Blues", visible=visible, colorbar=dict(title="mean p"),
        )
    ...
    buttons = [
        dict(label="p_faith", method="update",
             args=[{"visible": [True, False]}, {"title": "среднее p_faith: kind × variant"}]),
        dict(label="p_rel", method="update",
    ...
    fig.update_layout(
        ...
        updatemenus=[dict(buttons=buttons, direction="down", x=1.0, xanchor="right",
                          y=1.18, showactive=True)],
        height=380, margin=dict(l=120, r=20, t=70, b=80),
    )
...
def _sec_calibration(idx: ResultsIndex, first: bool) -> tuple[str, bool]:
    ...
    fig.add_trace(go.Scatter(x=[0, 1], y=[0, 1], mode="lines",
                             line=dict(dash="dash", color="gray"), name="идеал"))
    for v in sorted(df["variant"].unique()):
        sub = df[df["variant"] == v]
        bins = reliability_bins(list(sub["p_faith"].astype(float)),
                                list(sub["faith"].astype(int)))
        filled = [b for b in bins if b["n"] > 0]
        if not filled:
            continue
        fig.add_trace(go.Scatter(
            x=[b["mean_prob"] for b in filled], y=[b["frac_pos"] for b in filled],
            mode="lines+markers", name=v,
            text=[f'n={b["n"]}' for b in filled], hovertemplate="%{text}<br>"
    ...
    fig.update_layout(
        ...
        height=460, margin=dict(l=50, r=20, t=50, b=50),
    )
...
def _sec_f1_curves(idx: ResultsIndex, first: bool) -> tuple[str, bool]:
    ...
    for a_i, (gold_col, p_col) in enumerate(axes):
        ...
        for v in variants:
            ...
            if sub.empty:
                continue
            ts, f1s = f1_threshold_curve(list(sub[p_col].astype(float)),
                                         list(sub[gold_col].astype(int)))
            fig.add_trace(go.Scatter(
                x=ts, y=f1s, mode="lines", name=v, visible=(a_i == 0),
    ...
    buttons = [
        dict(label="faith", method="update",
             args=[{"visible": vis_faith}, {"title": "f1(t): ось faith"}]),
        dict(label="rel", method="update",
             args=[{"visible": vis_rel}, {"title": "f1(t): ось rel"}]),
    ]
    fig.update_layout(
        title="f1(t): ось faith",
        updatemenus=[dict(buttons=buttons, direction="down", x=1.0, xanchor="right",
                          y=1.15, showactive=True)],
        xaxis=dict(title="порог t", range=[0, 1]), yaxis=dict(title="f1-macro", range=[0, 1]),
        height=440, margin=dict(l=50, r=20, t=60, b=50),
    )
...
def _sec_confusion(idx: ResultsIndex, first: bool) -> tuple[str, bool]:
    ...
    for i, (v, c) in enumerate(tiles):
        ...
        fig.add_trace(go.Heatmap(
            z=z, text=text, texttemplate="%{text}", showscale=False,
            colorscale="Blues", x=["pred 0", "pred 1"], y=["gold 0", "gold 1"],
        ), row=row, col=col)
    fig.update_layout(height=240 * nrow, margin=dict(l=40, r=20, t=50, b=30),
...
def _sec_gepa(idx: ResultsIndex, first: bool) -> tuple[str, bool]:
    ...
    for variant, gv in ev.groupby("variant"):
        ...
        for s_i, seed in enumerate(sorted(gv["seed"].dropna().unique())):
            ...
            fig.add_trace(go.Scatter(
                x=list(gs["cand_idx"]), y=list(gs["val_score"]), mode="lines+markers",
                name=f"{variant} s{int(seed)}", line=dict(color=color),
            ...
            if not best.empty:
                fig.add_trace(go.Scatter(
                    x=list(best["cand_idx"]), y=list(best["val_score"]), mode="markers",
                    marker=dict(symbol="star", size=16, color=color,
                                line=dict(color="black", width=1)),
                    name=f"best {variant} s{int(seed)}", legendgroup=variant,
                    showlegend=False,
                ), row=1, col=1)
    ...
    if have_budget:
        ...
        fig.add_trace(go.Bar(x=labels, y=list(budg["task_lm_calls"]), name="task_lm_calls",
                             marker_color="#54a24b"), row=1, col=2)
        fig.add_trace(go.Bar(x=labels, y=list(budg["reflection_lm_calls"]),
                             name="reflection_lm_calls", marker_color="#b279a2"), row=1, col=2)
    fig.update_xaxes(title_text="cand_idx", row=1, col=1)
    fig.update_yaxes(title_text="val_score", row=1, col=1)
    fig.update_layout(height=460, barmode="group", margin=dict(l=50, r=20, t=60, b=60),
...
def _sec_m6(idx: ResultsIndex, first: bool) -> tuple[str, bool]:
    ...
    fig = make_subplots(rows=1, cols=2, column_widths=[0.55, 0.45],
                        subplot_titles=["contra_mean × semantic_entropy",
                                        "Δ semantic_entropy: thr × n"])
    # scatter
    if feats is not None and not feats.empty:
        ...
        for k_i, kind in enumerate(KIND_ORDER + ["?"]):
            ...
            if sub.empty:
                continue
            fig.add_trace(go.Scatter(
                x=list(sub["selfcheck_contra_mean"]), y=list(sub["semantic_entropy"]),
                mode="markers", name=kind,
                marker=dict(size=[8 + 2 * (n or 0) for n in sub["n_clusters"]],
                            color=palette[k_i % len(palette)], opacity=0.75),
                text=list(sub["id"]),
    ...
    if abl is not None and not abl.empty:
        piv = abl.pivot_table(index="thr", columns="n", values="delta_semantic_entropy",
                              aggfunc="mean").sort_index()
        text = [[f"{v:.3f}" if pd.notna(v) else "" for v in row] for row in piv.values]
        fig.add_trace(go.Heatmap(
            z=piv.values.tolist(), x=[str(c) for c in piv.columns],
            y=[str(i) for i in piv.index], text=text, texttemplate="%{text}",
            colorscale="RdBu", zmid=0, colorbar=dict(title="Δ SE", x=1.0),
        ), row=1, col=2)
        fig.update_xaxes(title_text="n сэмплов", row=1, col=2)
        fig.update_yaxes(title_text="entail_threshold", row=1, col=2)
    fig.update_layout(height=460, margin=dict(l=50, r=20, t=60, b=60),
...
def _sec_metadata(idx: ResultsIndex, first: bool) -> tuple[str, bool]:
    ...
    if runs is not None and not runs.empty:
        ...
        rv = runs[rcols].copy()
        rv["git_hash"] = rv["git_hash"].apply(
            lambda h: str(h)[:10] if isinstance(h, str) else "—")
    ...
    fig.add_trace(go.Table(
        header=dict(values=[f"<b>{c}</b>" for c in rcols], fill_color="#2b3a55",
    ...
    if preds is not None and not preds.empty and "extract_method" in preds.columns:
        ...
        if not em.empty:
            share = (em.groupby(["variant", "extract_method"]).size()
                     .rename("n").reset_index())
            for method in sorted(share["extract_method"].unique()):
                sm = share[share["extract_method"] == method]
                fig.add_trace(go.Bar(x=list(sm["variant"]), y=list(sm["n"]),
                                     name=str(method)), row=1, col=2)
            fig.update_yaxes(title_text="кейсов", row=1, col=2)
    fig.update_layout(height=420, barmode="stack", margin=dict(l=20, r=20, t=50, b=80),
...
def build_report(idx: ResultsIndex, splits: list[str]) -> str:
    ...
    for builder in (_sec_leaderboard, _sec_grouped_bar, _sec_box, _sec_heatmap_kind,
                    _sec_calibration, _sec_f1_curves, _sec_confusion, _sec_gepa,
                    _sec_m6, _sec_metadata):
...
def _filter_splits(idx: ResultsIndex, splits: list[str]) -> ResultsIndex:
    ...
    return ResultsIndex(
        runs=flt(idx.runs), predictions=flt(idx.predictions),
        m6_features=flt(idx.m6_features), gepa=idx.gepa,
