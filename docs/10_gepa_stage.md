...
| `tools/make_figs.py` | боксплоты p по kind, reliability diagram, f1(t), scatter m6 | `tests/test_figs_logic.py` (2) |
| `src/m3/run_gepa.py` | DSPy/GEPA-оптимизация инструкции судьи; метрика ±маркеры; глосс из yaml | `tests/test_gepa_metric.py` (5) |
| `src/m3/gepa_report.py` | markdown-отчёт эволюции промпта из track_stats | I/O-глю, без юнит-теста |
| `tools/entropy_ablation.py` | абляция entropy по (порог × N) из кэша, 0 LLM-вызовов | `tests/test_entropy_ablation.py` (2) |

### Изменённый код

| Файл | Изменение |
|---|---|
| `src/m3/predict.py` | `--prompt-file` (override пути промпта для mode=gepa) и `--variant-name` (поддиректория predictions, чтобы прогоны markers/plain×seed не перетирали друг друга) |
| `src/m6/sample.py` | `--n` (override `m6.n_samples`; нужен для добора до 10 сэмплов под абляцию) |
...
`tools/entropy_ablation.py` считает NLI-матрицу **один раз на кейс** (полная матрица пар
...
python -m src.m3.run_gepa --config configs/config.cloud.yaml --variant markers --seed 0 \
    --train-size 50 --auto light
python -m src.m3.run_gepa --config configs/config.cloud.yaml --variant markers --seed 0 --train-size 100
#   … markers/plain × seed 0/1, затем инференс:
python -m src.m3.predict --config configs/config.cloud.yaml --mode gepa \
...
python -m src.m6.sample   --config configs/config.cloud.yaml --split val --n 10
