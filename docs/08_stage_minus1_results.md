...
**Абляция N=10 (правка B4).** Проверяли гипотезу «на n=5 энтропия неинформативна из-за
малого числа сэмплов; при n=10 сигнал вырастет». Гипотеза **опровергнута**: при N=10
entropy Δ (halluc − clean) стал **−0.164** (было +0.009) — не вырос, а инвертировался.
Причина видна в средних: при N=10 у clean больше кластеров (5.18) и выше энтропия (1.198),
чем у hallucination (4.0 / 1.034) — «чистые» ответы расходятся на парафразы, а галлюцинации
кучнее. То есть semantic entropy на этом корпусе не просто слаба, а при росте N уводит в
неверную сторону; contradiction (Δ+0.893 при N=10) остаётся единственным рабочим сигналом
faithfulness. Дефолт вернули к n=5 (N=10 вдвое дороже без пользы). Код добора сэмплов
(`_need_samples`) сохранён — кэш можно доращивать без пересэмплирования.
